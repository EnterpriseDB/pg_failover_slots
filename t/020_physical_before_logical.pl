...
$node_primary->append_conf('postgresql.conf', "shared_preload_libraries = edb_failover_slots");
# Setup physical before logical slot
$node_primary->append_conf('postgresql.conf', "edb_failover_slots.standby_slot_names = 'standby_1'");
...
my $node_primary_connstr = $node_primary->connstr . ' dbname=postgres';
...
$offset = $node_primary->wait_for_log(
        qr/terminating walsender process due to edb_failover_slots.standby_slot_names replication timeout/,
        0);
