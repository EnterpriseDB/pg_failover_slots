...
static char *edb_standby_slot_names_raw;
static char *edb_standby_slot_names_string = NULL;
List *edb_standby_slot_names = NIL;
int edb_standby_slots_min_confirmed;
XLogRecPtr edb_standby_slot_names_oldest_flush_lsn = InvalidXLogRecPtr;

/* Slots to sync */
char *edb_failover_slots_dsn;
char *edb_failover_slot_names;
...
static void
edb_assign_failover_slot_names(const char *newval, void *extra)
{
	...
	if (edb_failover_slot_names_str)
		pfree(edb_failover_slot_names_str);
	if (edb_failover_slot_names_list)
	{
		foreach (lc, edb_failover_slot_names_list)
		{
			FailoverSlotFilter *filter = lfirst(lc);

			/* val was pointer to edb_failover_slot_names_str */
			pfree(filter);
		}
		list_free(edb_failover_slot_names_list);
	}

	edb_failover_slot_names_list = NIL;

	/* Allocate memory in long lasting context. */
	old_ctx = MemoryContextSwitchTo(TopMemoryContext);

	edb_failover_slot_names_str = pstrdup(newval);
	SplitIdentifierString(edb_failover_slot_names_str, ',', &slot_names_list);
	...
	foreach (lc, slot_names_list)
	{
		...
		edb_failover_slot_names_list =
			lappend(edb_failover_slot_names_list, filter);
	}
...
static bool
edb_check_standby_slot_names(char **newval, void **extra, GucSource source)
{
	List *namelist = NIL;
	char *rawname = pstrdup(*newval);
	bool valid;
...
static void
edb_assign_standby_slot_names(const char *newval, void *extra)
{
	MemoryContext old_ctx;

	if (edb_standby_slot_names_string)
		pfree(edb_standby_slot_names_string);
	if (edb_standby_slot_names)
		list_free(edb_standby_slot_names);

	/*
	 * We must invalidate our idea of the oldest lsn in all the named slots if
	 * we might have changed the list.
	 */
	edb_standby_slot_names_oldest_flush_lsn = InvalidXLogRecPtr;

	old_ctx = MemoryContextSwitchTo(TopMemoryContext);
	edb_standby_slot_names_string = pstrdup(newval);
	(void) SplitIdentifierString(edb_standby_slot_names_string, ',',
								 &edb_standby_slot_names);
...
{
	if (edb_failover_slots_dsn && strlen(edb_failover_slots_dsn) > 0)
	{
		if (db_name)
			appendStringInfo(connstr, "%s dbname=%s", edb_failover_slots_dsn,
							 db_name);
		else
			appendStringInfoString(connstr, edb_failover_slots_dsn);
	}
	else
	{
		Assert(WalRcv);
...
wait_for_primary_slot_catchup(ReplicationSlot *slot, RemoteSlot *remote_slot)
{
	...
	conn = remote_connect(connstr.data, "edb_failover_slots");
	pfree(connstr.data);
...
synchronize_failover_slots(long sleep_time)
{
	...
	if (!WalRcv || !HotStandbyActive() ||
		list_length(edb_failover_slot_names_list) == 0)
		return sleep_time;
	 ...
	 * promotion
	 */
	slots = remote_get_primary_slot_info(conn, edb_failover_slot_names_list);
	...
	for (;;)
	{
		...
		for (i = 0; i < max_replication_slots; i++)
		{
			 ...
			 * is enabled.
			 */
			if (!found && edb_failover_slots_drop)
			{
...
void
edb_failover_slots_main(Datum main_arg)
{
	...
	elog(LOG, "starting edb_failover_slots replica worker");
...
skip_standby_slot_names(XLogRecPtr commit_lsn)
{
	...
	if (edb_standby_slot_names != cached_standby_slot_names)
	{
		if (MyReplicationSlot)
		{
			if (list_member_str(edb_standby_slot_names,
								NameStr(MyReplicationSlot->data.name)))
			{
				edb_standby_slots_min_confirmed = 0;
				elog(
					DEBUG1,
					"found my slot in edb_failover_slots.standby_slot_names, no need to wait for confirmations");
			}
		}

		cached_standby_slot_names = edb_standby_slot_names;
	}
	...
	if (edb_standby_slot_names_oldest_flush_lsn >= commit_lsn ||
		edb_standby_slots_min_confirmed == 0 ||
		list_length(edb_standby_slot_names) == 0)
		return true;
...
wait_for_standby_confirmation(XLogRecPtr commit_lsn)
{
	...
	while (1)
	{
		...
		if (wait_slots_remaining == 0)
		{
			 ...
			 * if the GUCs change.
			 */
			if (edb_standby_slot_names_oldest_flush_lsn < oldest_flush_pos)
				edb_standby_slot_names_oldest_flush_lsn = oldest_flush_pos;
				...
				TimestampTzPlusMilliseconds(wait_start, wal_sender_timeout))
		{
			ereport(
				COMMERROR,
				(errmsg(
					"terminating walsender process due to edb_failover_slots.standby_slot_names replication timeout")));
			proc_exit(0);
		}
...
static void
edb_socket_comm_reset(void)
{
	OldPqCommMethods->comm_reset();
}

static int
edb_socket_flush(void)
{
	return OldPqCommMethods->flush();
}

static int
edb_socket_flush_if_writable(void)
{
	return OldPqCommMethods->flush_if_writable();
}

static bool
edb_socket_is_send_pending(void)
{
...
static int
edb_socket_putmessage(char msgtype, const char *s, size_t len)
{
	return OldPqCommMethods->putmessage(msgtype, s, len);
}

static void
edb_socket_putmessage_noblock(char msgtype, const char *s, size_t len)
{
	if (msgtype == 'd' && len >= 17)
	{
		if (s[0] == 'w')
...
_PG_init(void)
{
	...
	DefineCustomStringVariable(
		...
		"replication.",
		&edb_standby_slot_names_raw, "", PGC_SIGHUP, GUC_LIST_INPUT,
		edb_check_standby_slot_names, edb_assign_standby_slot_names, NULL);


	DefineCustomIntVariable(
		"edb_failover_slots.standby_slots_min_confirmed",
		"Number of slots from edb_failover_slots.standby_slot_names that must confirm lsn",
		"Modifies behaviour of edb_failover_slots.standby_slot_names so to allow "
		"logical replication of a transaction after at least "
		"edb_failover_slots.standby_slots_min_confirmed physical peers have confirmed "
		"the transaction as durably flushed. "
		"The value -1 (default) means all entries in edb_failover_slots.standby_slot_names"
		"must confirm the write. The value 0 causes "
		"edb_failover_slots.standby_slots_min_confirmedto be effectively ignored.",
		&edb_standby_slots_min_confirmed, -1, -1, 100, PGC_SIGHUP, 0, NULL,
		NULL, NULL);

	DefineCustomStringVariable(
		"edb_failover_slots.synchronize_slot_names",
		"list of slots to synchronize from primary to physical standby", "",
		&edb_failover_slot_names, "name_like:%%",
		PGC_SIGHUP, /* Sync ALL slots by default */
		GUC_LIST_INPUT, edb_check_failover_slot_names,
		edb_assign_failover_slot_names, NULL);


	DefineCustomBoolVariable(
		"edb_failover_slots.drop_extra_slots",
		"whether to drop extra slots on standby that don't match edb_failover_slots.synchronize_slot_names",
		NULL, &edb_failover_slots_drop, true, PGC_SIGHUP, 0, NULL, NULL, NULL);
	...
	snprintf(bgw.bgw_function_name, BGW_MAXLEN, "edb_failover_slots_main");
	snprintf(bgw.bgw_name, BGW_MAXLEN, "edb_failover_slots worker");
